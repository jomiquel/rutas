var miCuadroRutas = {
	...
	dimensiones: {
		...
		calcular: function(puntos) {
			...
			{
				bottom = Math.min(bottom, puntos[i].lat);
				top = Math.max(top, puntos[i].lat);
				left = Math.min(left, puntos[i].lng);
				right = Math.max(right, puntos[i].lng);
			}
